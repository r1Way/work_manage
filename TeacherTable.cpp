...
TeacherTable::TeacherTable()
{
    ...
    while (query.next())
    {
        addTeacher(query.value("id").toString(),query.value("name").toString(),query.value("major").toString());
    }
    ...
    connect(add,&QPushButton::clicked,[this]()
            {
                ...
                connect(ensureAdd,&QPushButton::clicked,[this,nameEdit,idEdit,majorEdit,dialog]()
                        {
                            this->addTeacher(nameEdit->text(),idEdit->text(),majorEdit->text());
                            QString sql=QString("insert into teacher(id,name,major) value('%1','%2','%3')").arg(idEdit->text()).arg(nameEdit->text()).arg(majorEdit->text());
                            QSqlQuery queryAdd;
                            queryAdd.exec(sql);
                            dialog->close();
                        });
                    ...
                    connect(ensureBatch,&QPushButton::clicked,[this,cols]()
                            {
                                for (int i = tableWidget->rowCount() - 1; i >= 0; --i)
                                {
                                    ...
                                    if (checkBox && checkBox->isChecked())
                                    {
                                        ...
                                        //sql中删除
                                        QString sql=QString("delete from teacher where id='%1';").arg(id);
